...
var draw: *const fn (state: *anyopaque) callconv(.C) void = undefined;
...
fn load_lib() !void {
    ...
    draw = lib.?.lookup(@TypeOf(draw), "draw").?;
...
fn update_hot_lib_path() !void {
    ...
    hot_lib_path = try std.fmt.allocPrint(std.heap.c_allocator, "{s}/game_{d}.{s}", .{ LIB_DIR, std.time.milliTimestamp(), ext });
}
